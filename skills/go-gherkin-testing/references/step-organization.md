---
description: How to organise step definitions by domain concept, avoid feature-coupling, reduce duplication with helper methods, and scale your Gherkin test suites.
source: https://cucumber.io/docs/gherkin/step-organization/
source_raw: https://raw.githubusercontent.com/cucumber/website/5af617a1434e7bfd577f1f9d68fae5e64d74fbe6/docs/gherkin/step-organization.mdx
---
...
## How godog finds your features and step definitions

Be aware that, regardless of the directory structure employed, godog effectively flattens the `features/` directory tree when running tests.
This means that anything matching the step definition file pattern inside the directory in which godog is run is treated as a step definition.
...
Instead, we recommend creating a separate step definition file for each domain concept.

A good rule of thumb is to have one file for each major *domain object*.

For example, in a Go project you might have:
...
The first three files would define all the `Given`, `When`, and `Then` step definitions related to creating, reading, updating, and deleting the various types of objects.
...
If you follow this pattern, you also avoid the [Feature-coupled step definitions](anti-patterns.md) anti-pattern.
...
And the following step definition in Go:

```go
package steps

func RegisterNavigationSteps(ctx *godog.ScenarioContext, s *State) {
	ctx.Step(`^I go to the "([^"]*)" page$`, func(webpage string) error {
		return webpageFactory.OpenPage(webpage)
	})
}
```
...
Also, using Data Tables for providing inputs to steps helps increase maintainability and understandability.

## Helper methods

Always keep in mind that godog step definitions are regular Go code whose full expressiveness
...
Gherkin and therefore must conform to the same syntax as used in feature files.

In fact, it is recommended to refactor step definitions into helper methods for greater modularity and reuse.
The helper method can reside in the same `.go` file as the step registration.
