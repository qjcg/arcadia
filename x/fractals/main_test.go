...
func TestMandelbrot(t *testing.T) {
	tests := []struct {
		...
		ci       float64
		maxIter  int
		expected bool // true if should be in set (reach maxIter)
	}{
		{
			name:     "Origin (0,0) is in set",
			cr:       0.0,
			ci:       0.0,
			maxIter:  50,
			expected: true,
		...
		{
			name:     "Distant point (2,0) diverges quickly",
			cr:       2.0,
			ci:       0.0,
			maxIter:  50,
			expected: false,
		...
		{
			name:     "Distant point (0,2) diverges quickly",
			cr:       0.0,
			ci:       2.0,
			maxIter:  50,
			expected: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := mandelbrot(tt.cr, tt.ci, tt.maxIter)

			if tt.expected {
				// Point should be in the set (reach maxIter)
				if iter != tt.maxIter {
					t.Errorf("Expected point (%v,%v) to be in set (iter=%d), got iter=%d",
						tt.cr, tt.ci, tt.maxIter, iter)
				}
			} else {
				// Point should diverge (not reach maxIter)
				if iter == tt.maxIter {
					t.Errorf("Expected point (%v,%v) to diverge before %d iterations",
						tt.cr, tt.ci, tt.maxIter)
				}
			}
		})
	}
...
func TestMandelbrotQuickDivergence(t *testing.T) {
	// Points far from the origin should diverge very quickly (within a few iterations)
	tests := []struct {
		name     string
		cr       float64
		ci       float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := mandelbrot(tt.cr, tt.ci, 50)
			if iter > tt.maxIters {
				t.Errorf("Expected point (%v,%v) to diverge within %d iterations, took %d",
					tt.cr, tt.ci, tt.maxIters, iter)
			}
		})
	}
}
...
func TestJulia(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Classic Julia set c=-0.7+0.27015i, origin is in set",
			zr:       0.0,
			zi:       0.0,
			cr:       -0.7,
			ci:       0.27015,
			maxIter:  50,
			expected: true, // Origin is in the set for this c
		},
		{
			name:     "Distant point diverges quickly",
			zr:       10.0,
			zi:       0.0,
			cr:       -0.7,
			ci:       0.27015,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := julia(tt.zr, tt.zi, tt.cr, tt.ci, tt.maxIter)

			if tt.expected {
				if iter != tt.maxIter {
					t.Errorf("Expected point (%v,%v) to be in set (iter=%d), got iter=%d",
						tt.zr, tt.zi, tt.maxIter, iter)
				}
			} else {
				if iter == tt.maxIter {
					t.Errorf("Expected point (%v,%v) to diverge before %d iterations",
						tt.zr, tt.zi, tt.maxIter)
				}
			}
		})
	}
...
func TestBurningShip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := burningShip(tt.cr, tt.ci, tt.maxIter)

			if tt.expected {
				if iter != tt.maxIter {
					t.Errorf("Expected point (%v,%v) to be in set, got iter=%d",
						tt.cr, tt.ci, iter)
				}
			} else {
				if iter == tt.maxIter {
					t.Errorf("Expected point (%v,%v) to diverge",
						tt.cr, tt.ci)
				}
			}
		})
...
func TestTricorn(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := tricorn(tt.cr, tt.ci, tt.maxIter)
...
func TestMultibrot3(t *testing.T) {
	tests := []struct {
		name     string
		cr       float64
		ci       float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := multibrot3(tt.cr, tt.ci, tt.maxIter)
...
func TestMultibrot4(t *testing.T) {
	tests := []struct {
		name     string
		cr       float64
		ci       float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := multibrot4(tt.cr, tt.ci, tt.maxIter)
...
func TestCeltic(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := celtic(tt.cr, tt.ci, tt.maxIter)
...
func TestPerpendicular(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := perpendicular(tt.cr, tt.ci, tt.maxIter)
...
func TestMultibrot5(t *testing.T) {
	tests := []struct {
		name     string
		cr       float64
		ci       float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := multibrot5(tt.cr, tt.ci, tt.maxIter)
...
func TestManhattan(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := manhattan(tt.cr, tt.ci, tt.maxIter)
...
func TestNewton(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Real root (1,0) converges",
			zr:      0.9,
			zi:      0.0,
			maxIter: 50,
		},
		{
			name:    "Complex root converges",
			zr:      -0.4,
			zi:      0.8,
			maxIter: 50,
		},
		{
			name:    "Point near first root",
			zr:      0.8,
			zi:      0.1,
			maxIter: 50,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := newton(tt.zr, tt.zi, tt.maxIter)

			// Newton fractal should return a valid iteration count
			if iter < 0 || iter > tt.maxIter {
				t.Errorf("newton() returned invalid iteration count: %d", iter)
			}

			// Most reasonable starting points should converge
			// (return non-zero iteration count)
			if iter == 0 {
				// This is valid but rare - just log it
				t.Logf("Point (%v,%v) did not converge (returned 0)", tt.zr, tt.zi)
			}
		})
	}
}
...
func TestNewtonConvergesToRoots(t *testing.T) {
	// Test that Newton's method converges to the three cube roots of unity
	// We're not testing exact convergence since the function returns based on
	// iteration counts, but we verify it produces different results for
	// different starting positions

	maxIter := 50

	// Points near each of the three roots should produce different results
	results := make(map[int]bool)

	// Near root 1 (1, 0)
	iter1 := newton(0.9, 0.0, maxIter)
	results[iter1] = true

	// Near root 2 (-0.5, 0.866)
	iter2 := newton(-0.45, 0.75, maxIter)
	results[iter2] = true

	// Near root 3 (-0.5, -0.866)
	iter3 := newton(-0.45, -0.75, maxIter)
	results[iter3] = true

	// Should have at least 2 different convergence behaviors
	// (allowing for some variance in the basin boundaries)
	if len(results) < 2 {
		t.Errorf("newton() produced only %d distinct results for different roots, expected at least 2",
			len(results))
	}

	// Check that points converge (non-zero results)
	converged := 0
	if iter1 > 0 {
		converged++
	}
	...
	if converged < 2 {
		t.Errorf("newton() only converged for %d out of 3 test points, expected at least 2", converged)
	}
}

func TestNewtonDoesNotPanic(t *testing.T) {
	// Test various edge cases to ensure Newton doesn't panic
	testCases := []struct {
		zr, zi float64
	}{
		{0.0, 0.0},     // Origin
		{1.0, 0.0},     // Real root
		{-0.5, 0.866},  // Complex root
		{10.0, 10.0},   // Far point
	...
	for _, tc := range testCases {
		// Should not panic
		iter := newton(tc.zr, tc.zi, maxIter)

		// Should return a valid value
		if iter < 0 || iter > maxIter {
			t.Errorf("newton(%v, %v) returned invalid iter: %d", tc.zr, tc.zi, iter)
		}
	}
}
...
func TestNewtonEdgeCases(t *testing.T) {
	maxIter := 50
	...
	tests := []struct {
		...
		{"Exactly on root 1", 1.0, 0.0},
		{"Exactly on root 2", -0.5, 0.866025},
		{"Exactly on root 3", -0.5, -0.866025},
		{"Very close to origin", 0.001, 0.0},
		{"Origin", 0.0, 0.0},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iter := newton(tt.zr, tt.zi, maxIter)
			if iter < 0 || iter > maxIter {
				t.Errorf("newton(%v, %v) = %d, want value in [0, %d]",
					tt.zr, tt.zi, iter, maxIter)
			}
		})
	}
}
