...
type Renderer struct {
	...
	dynamicColor     bool
	hueShift         float64
	breakthroughTr   *transitions.Breakthrough
...
func (r *Renderer) SetBreakthroughTransition(tr *transitions.Breakthrough) {
	r.breakthroughTr = tr
}
...
func (r *Renderer) renderFromIterations(iterGrid [][]int) string {
	...
	for row := range r.config.Height {
		for col := range r.config.Width {
			...
			var color string
			if r.dynamicColor {
				color = colorthemes.GetColorWithHueShift(iter, r.config.MaxIter, r.config.ColorScheme, r.hueShift)
			} else {
				color = colorthemes.GetColor(iter, r.config.MaxIter, r.config.ColorScheme)
			}
	...
	for row := range r.config.Height {
		...
		if r.config.ColorScheme != colorthemes.ColorGrayscale {
