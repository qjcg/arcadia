...
import (
	...
	"github.com/qjcg/arcadia/exp/fractalis/internal/core/search"
	"github.com/qjcg/arcadia/exp/fractalis/internal/tui/animation"
	"github.com/qjcg/arcadia/exp/fractalis/internal/tui/color"
	renderlib "github.com/qjcg/arcadia/exp/fractalis/internal/tui/render"
)
...
func TestMapToComplex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cr, ci := renderlib.MapToComplex(tt.col, tt.row, width, height, centerX, centerY, zoom)
...
func TestMapToComplexZoom(t *testing.T) {
	...
	cr1, _ := renderlib.MapToComplex(0, height/2, width, height, centerX, centerY, 1.0)
	cr2, _ := renderlib.MapToComplex(0, height/2, width, height, centerX, centerY, 2.0)
...
func TestGetChar(t *testing.T) {
	...
	for i := 0; i < maxIter; i += 5 {
		char := renderlib.GetChar(i, maxIter)
		chars[char] = true
	}
...
func TestGetColor(t *testing.T) {
	...
	color1 := color.GetColor(25, maxIter, color.ColorGrayscale)
	if color1 != "" {
		t.Errorf("Expected empty string for grayscale, got %q", color1)
	}

	// Test blue returns ANSI color code
	color2 := color.GetColor(25, maxIter, color.ColorBlue)
	if color2 == "" {
		t.Error("Expected ANSI color code for blue scheme, got empty string")
	}

	// Test rainbow returns ANSI color code
	color3 := color.GetColor(25, maxIter, color.ColorRainbow)
	if color3 == "" {
		t.Error("Expected ANSI color code for rainbow scheme, got empty string")
	}

	// Test points in set get black
	color4 := color.GetColor(maxIter, maxIter, color.ColorBlue)
	expected := "\033[38;5;0m"
	if color4 != expected {
		t.Errorf("Expected black color %q for points in set, got %q", expected, color4)
	}
}
...
func TestCalculateFractal(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			iter := fractal.CalculateFractal(tt.cr, tt.ci, tt.config.FractalType, tt.config.MaxIter, tt.config.JuliaCr, tt.config.JuliaCi)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			calculator := search.NewInterestCalculator(
				func(cr, ci float64, cfg persistence.Config) float64 {
...
func TestCalculateInterestScore(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create an interest calculator directly
			calculator := search.NewInterestCalculator(
				func(cr, ci float64, cfg persistence.Config) float64 {
					return fractal.CalculateFractal(cr, ci, cfg.FractalType, cfg.MaxIter, cfg.JuliaCr, cfg.JuliaCi)
				},
			)
			got := calculator.CalculateScore(tt.cx, tt.cy, persistence.Config(tt.config))

			// Check if score is within expected range
			if got < tt.wantScore-tt.tolerance {
				t.Errorf("CalculateScore() = %v, want at least %v", got, tt.wantScore)
			}
		})
	}
}
...
func TestLoadBookmark(t *testing.T) {
	m := model{
		...
		bookmarks: []persistence.Bookmark{
			{
				Name: "test1",
				URL:  "fractal://julia/-0.7/0.27/10.0/100/?color_theme=blue&julia_cr=-0.8&julia_ci=0.156",
			},
			{
				Name: "test2",
				URL:  "fractal://burningship/-0.5/-0.6/5.0/75/?color_theme=rainbow",
			},
		},
	}
	...
	if m.config.ColorScheme != color.ColorBlue {
		t.Errorf("loadBookmark() ColorScheme = %s, want %s", m.config.ColorScheme, color.ColorBlue)
	}
...
func TestDeleteBookmark(t *testing.T) {
	// Use temp HOME to avoid loading existing bookmarks from user's config
	tmpDir := t.TempDir()
	originalPath := os.Getenv("HOME")
	defer os.Setenv("HOME", originalPath)
	os.Setenv("HOME", tmpDir)

	// Pre-save bookmarks to disk so deleteBookmark can reload them
	initialBookmarks := []persistence.Bookmark{
		{Name: "test1", URL: "fractal://mandelbrot/-0.5/0.0/1.0/50/"},
		{Name: "test2", URL: "fractal://julia/0.0/0.0/1.0/50/"},
		{Name: "test3", URL: "fractal://burningship/0.0/0.0/1.0/50/"},
	}
	if err := persistence.SaveBookmarks(initialBookmarks); err != nil {
		t.Fatalf("Failed to setup test bookmarks: %v", err)
	}
...
func TestAllFractalTypesValid(t *testing.T) {
	...
	for _, fractalType := range allFractalTypes {
		...
		_ = fractal.CalculateFractal(0.0, 0.0, config.FractalType, config.MaxIter, config.JuliaCr, config.JuliaCi)
	}
}

func TestAllColorSchemesValid(t *testing.T) {
	// Test that all color schemes work
	colorSchemes := []string{color.ColorGrayscale, color.ColorBlue, color.ColorRainbow}

	for _, scheme := range colorSchemes {
		color := color.GetColor(25, 50, scheme)
...
func TestBookmarkYAMLMarshaling(t *testing.T) {
	...
	// Bookmarks now only store Name and URL - all other data is in the URL
	bookmarks := []persistence.Bookmark{
		{
			Name: "test_bookmark",
			URL:  "fractal://newton/0.5/-0.3/42.5/150/?color_theme=rainbow",
		},
		{
			Name: "julia_test",
			URL:  "fractal://julia/0.0/0.0/1.0/50/?color_theme=blue&julia_cr=-0.8&julia_ci=0.156",
		},
	}
	...
	for _, bm := range bookmarks {
		...
		if bm.URL == "" {
			t.Error("Bookmark has empty URL")
		}
		// Verify URL can be parsed
		if _, err := persistence.ParseFractalURL(bm.URL); err != nil {
...
func TestMapToComplexSymmetry(t *testing.T) {
	...
	crCenter, ciCenter := renderlib.MapToComplex(width/2, height/2, width, height, centerX, centerY, zoom)
	...
	crLeft, _ := renderlib.MapToComplex(0, height/2, width, height, centerX, centerY, zoom)
	crRight, _ := renderlib.MapToComplex(width-1, height/2, width, height, centerX, centerY, zoom)
...
func TestGetCharBoundaries(t *testing.T) {
	...
	for _, tt := range tests {
		char := renderlib.GetChar(tt.iter, maxIter)
		if tt.iter == 0 && char != ' ' {
			t.Errorf("renderlib.GetChar(0) = %c, want ' '", char)
		}
		if tt.iter == maxIter && char != '@' {
			t.Errorf("renderlib.GetChar(maxIter) = %c, want '@'", char)
		}
	}
	...
	for i := 0; i <= maxIter; i += 10 {
		...
		if !found {
			t.Errorf("renderlib.GetChar(%d) = %c is not in valid character set", i, char)
		}
	}
}
...
func TestGetColorAllSchemes(t *testing.T) {
	...
	tests := []struct {
		...
		{"Grayscale low iter", color.ColorGrayscale, 5},
		{"Grayscale mid iter", color.ColorGrayscale, 25},
		{"Grayscale high iter", color.ColorGrayscale, 45},
		{"Blue low iter", color.ColorBlue, 5},
		{"Blue mid iter", color.ColorBlue, 25},
		{"Blue high iter", color.ColorBlue, 45},
		{"Blue at maxIter", color.ColorBlue, maxIter},
		{"Rainbow low iter", color.ColorRainbow, 5},
		{"Rainbow mid iter", color.ColorRainbow, 25},
		{"Rainbow high iter", color.ColorRainbow, 45},
		{"Rainbow at maxIter", color.ColorRainbow, maxIter},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := color.GetColor(tt.iter, maxIter, tt.scheme)

			if tt.scheme == color.ColorGrayscale {
				if result != "" {
					t.Errorf("Expected empty string for grayscale, got %q", result)
				}
			} else {
				// Blue and rainbow should return ANSI codes
				if result == "" && tt.iter != maxIter {
...
func TestFractalQuickDivergence(t *testing.T) {
	...
	for _, fractalType := range fractals {
		t.Run(fractalType, func(t *testing.T) {
			...
			iter := fractal.CalculateFractal(farPoint, farPoint, config.FractalType, config.MaxIter, config.JuliaCr, config.JuliaCi)
...
func TestAddBookmarkIntegration(t *testing.T) {
	// Test bookmark structure - bookmarks now only store Name and URL
	bookmark := persistence.Bookmark{
		Name: "test_newton",
		URL:  "fractal://newton/1.5/-0.5/25.0/100/?color_theme=rainbow",
	}
	...
	// Verify URL can be parsed and contains expected values
	if params, err := persistence.ParseFractalURL(bookmark.URL); err == nil {
		if params.Zoom != 25.0 {
			t.Errorf("Bookmark zoom = %v, want 25.0", params.Zoom)
		}
		if params.FractalType != FractalNewton {
			t.Errorf("Bookmark fractal type = %s, want %s", params.FractalType, FractalNewton)
		}
	} else {
		t.Errorf("Failed to parse bookmark URL: %v", err)
	}
}
...
func TestRenderBookmarks(t *testing.T) {
	m := model{
		...
		bookmarks: []persistence.Bookmark{
			{Name: "test1", URL: "fractal://mandelbrot/-0.5/0.0/1.0/50/"},
			{Name: "test2", URL: "fractal://newton/0.0/0.0/50.0/100/"},
		},
		bookmarkCursor: 0,
	}
